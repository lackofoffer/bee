...
) TYPE=MyISAM COMMENT='管理员的密码和用户名';
CREATE TABLE `"$prefix"category` (
  ...
  `mid` smallint(1) NOT NULL default '1',
  PRIMARY KEY  (`cid`)
) TYPE=MyISAM COMMENT='保存站点信息' AUTO_INCREMENT=1 ;
...
CREATE TABLE `"$prefix"postruler` (
  ...
  PRIMARY KEY  (`pid`)
) TYPE=MyISAM COMMENT='设置模拟提交的规则' AUTO_INCREMENT=1 ;
...
CREATE TABLE `"$prefix"update` (
  ...
  PRIMARY KEY  (`upid`),
  UNIQUE KEY `url` (`url`)
) TYPE=MyISAM COMMENT='需要更新的记录' AUTO_INCREMENT=1 ;
CREATE TABLE `"$prefix"message` (
  ...
  PRIMARY KEY  (`mid`)
) TYPE=MyISAM COMMENT='保存采集内容' AUTO_INCREMENT=1;
begin;
?>